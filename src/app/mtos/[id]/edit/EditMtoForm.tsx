...
export default function EditMtoForm({ mto, staffList }: { mto: any, staffList: any[] }) {
  ...
  const [category, setCategory] = useState(mto.category);
  const [isStudded, setIsStudded] = useState(mto.isStudded ? 'true' : 'false');

  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setLoading(true);
    const formData = new FormData(e.currentTarget);
    const res = await updateMtoQuery(mto.id, formData);
    setLoading(false);
    if (res.success) {
      router.push(`/mtos/${mto.id}`);
      router.refresh();
    }
  }
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <header style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: 700 }}>Edit MTO Query</h1>
        <p style={{ color: 'var(--text-muted)' }}>Update customer intent and basic design requirements.</p>
      </header>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
            <div>
              ...
              <select name="leadType" defaultValue={mto.leadType}>
            ...
            {category === 'OTHER' && (
              <div>
                ...
                <input name="otherCategory" required defaultValue={mto.category} placeholder="e.g. Cufflinks" />
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: mtoType === 'REGULAR' ? '1fr 1fr' : '1fr', gap: '1rem' }}>
            {mtoType === 'REGULAR' && (
              <div>
                ...
                <input name="catalogueSku" required defaultValue={mto.catalogueSku || ''} placeholder="e.g. RING-1024" />
              </div>
            )}
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Update Reference Image / Sketch</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            <div>
              ...
              <select name="goldKaratage" defaultValue={mto.goldKaratage || '18K'}>
            ...
            <div>
              ...
              <select name="metalColor" required defaultValue={mto.metalColor || 'Yellow Gold'}>
            ...
            {isStudded === 'true' && (
              <div>
                ...
                <input name="diamondCaratage" defaultValue={mto.diamondCaratage || ''} placeholder="e.g. 1.5 Ct" />
              </div>
            )}
            <div>
              ...
              <input name="size" defaultValue={mto.size || ''} placeholder="e.g. Ring Size 12" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Approx Gold Weight</label>
              <input name="goldWeight" defaultValue={mto.goldWeight || ''} placeholder="e.g. ~12.5g" />
            </div>
          </div>

          <div>
             <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Design Notes / Customer Specs</label>
             <textarea name="notes" rows={4} defaultValue={mto.notes || ''} placeholder="Any specific requirements..."></textarea>
          </div>
